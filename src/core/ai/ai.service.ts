...
import { AIAnalysis, AnalysisId, ConfidenceScore, PageElement } from '../../types/ai/ai-analysis.entity';
import { AIService } from '../../types/ai/ai.service';
import { NotFoundError } from '../../types/common/errors/domain-error';
...
import { StrategyManagerService } from '../booking/strategy-manager.service';
...
export class AIServiceImpl implements AIService {
  ...
  constructor(
    ...
    this.llm = new ChatOpenAI({
      modelName: this.configService.get('AI_MODEL_NAME'),
  ...
  async analyzePage(html: string): Promise<AIAnalysis> {
    try {
      ...
      const analysis = new AIAnalysis(
        new AnalysisId(crypto.randomUUID()),
        [],
        new Date(),
        parsedResult
      );
      ...
      const responseChain = RunnableSequence.from([
        ...
        },
        {
          prompt: PromptTemplate.fromTemplate(`
            ...
            Provide a clear, concise, and relevant response.
          `),
        },
        this.llm,
        new StringOutputParser(),
  ...
  async validateInput(input: string): Promise<boolean> {
    try {
      ...
      this.loggingService.info('Input validation completed successfully');
      this.monitoringService.recordMetric({
